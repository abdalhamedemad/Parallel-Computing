...
,unsigned int * prev_forntier,  unsigned int * current_forntier, unsigned int * prev_forntier_size, unsigned int * current_forntier_index) {
    ...
    if(tx < *prev_forntier_size){
        ...
        // loop over the childs of the vertex
...
int main(int argc, char *argv[]) {
  ...
  // read adjacency matrix file name
    ...
    // starting node
    level[0] = 0;
    // to check if there is a new vertex visited
    ...
    // size of the forntier in the previous level
    unsigned int h_prev_forntier_size =1;

    // will loop until no elements in the previous forntier
