...
int main(int argc, char *argv[]) {
    ...
    cudaMemcpyAsync(d_row, csr.row.data(), csr.row_size * sizeof(unsigned int), cudaMemcpyHostToDevice, stream1);
    cudaMemcpyAsync(d_col, csr.col.data(), csr.col_size * sizeof(unsigned int), cudaMemcpyHostToDevice, stream2);
    cudaMemcpyAsync(d_row_size, &csr.row_size, sizeof(unsigned int), cudaMemcpyHostToDevice, stream1);
    cudaMemcpyAsync(d_col_size, &csr.col_size, sizeof(unsigned int), cudaMemcpyHostToDevice, stream2);
    cudaMemcpyAsync(d_level, level.data(), num_nodes * sizeof(unsigned int), cudaMemcpyHostToDevice, stream1);
    cudaMemcpyAsync(d_prev_forntier, &level[0], sizeof(unsigned int), cudaMemcpyHostToDevice, stream2);
    cudaMemsetAsync(d_prev_forntier_size, 1, sizeof(unsigned int), stream1);
    ...
    for (unsigned int current_level = 1; h_prev_forntier_size > 0; ++current_level) {
        ...
        cudaMemcpyAsync(&h_prev_forntier_size, d_current_forntier_index, sizeof(unsigned int), cudaMemcpyDeviceToHost, stream2);
